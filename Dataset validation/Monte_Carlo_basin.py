...
def init_worker(shared_data):
    """Initialize shared data for parallel processing"""
    global global_vars
    global_vars = shared_data

def inpolygon(lons, lats, basin_polygon):
    """Check if points fall within a basin polygon"""
    points = [Point(lon, lat) for lon,lat in zip(lons,lats)]
    return [basin_polygon.contains(point) for point in points]

def calculate_slope(date_collect,wse_collect):
    """Calculate water level fluctuations and change rates from basin-level water level time series"""
...
# Process single basin with Monte Carlo simulation
def process_single_basin(basin_ID):
    try:
       '''Workflow:
   1. Collect stations within basin
   2. Calculate original metrics
   ...
   Returns:
   - Dictionary with original and simulated metrics
   '''
        ...
        if count > 35:
            ...
            # implement 100 simulations
            for sim_index in range(100):
                #add errors to VS observations
                ...
                time_basin_collect_dates = [excel_num_to_date(date) for date in time_basin_collect]
                time_basin_collect_date_np = np.array(date2num(time_basin_collect_dates))
                #calculate the simulated basin-level flucyuations and change rates
